...
class TheTEAAdminSupabaseUser extends BaseAuthUser {
  TheTEAAdminSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> theTEAAdminSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TheTEAAdminSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
